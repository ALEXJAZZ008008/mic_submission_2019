...
\begin{abstract}
Respiratory motion decreases image quality in positron emission tomography. Unless gated CT or MR data are available, motion correction relies on image registration of the PET data. To avoid mis-registration due to attenuation mismatches, most existing methods rely on pair-wise registration of non-attenuation-corrected (NAC) PET images. This is a challenging problem due to a low contrast and high noise. This paper investigates the possibility of using motion models for respiratory motion correction in PET, and in particular whether incorporating time-of-flight information increases the accuracy of the motion model derived from the NAC reconstructed images. XCAT simulations are used for one bed position with a field of view of the lungs and diaphragm. A TOF resolution of 375ps was used. NAC images are obtained using OSEM and used as input for motion model estimation. Different motion models are compared using the XCAT ground truth images. The results indicate that TOF improves the accuracy of the motion model considerably.
...
Motion modelling attempts to overcome these deficiencies by relating the motion in the data to a surrogate signal~\cite{McClelland2013}.%, for instance, the displacement of the anterior surface and/or the position of the diaphragm.
...
3D B-splines were used to model spatial deformations with the corresponding warping operation denoted as $W(\alpha_t)$. Following~\cite{McClelland2013}, we use a motion model where the coefficients of the deformation field at time $t$ are expressed as a polynomial of the scalar $s_t$ (direct correspondence model):
\begin{equation}
    \alpha_{kt} = R_1 {s_t}^2 + R_2 s_t + R_3
\end{equation}
\noindent where $\alpha_{kt}$ is the 3D B-spline coefficient for node $k$ at time point $t$, $s_t$ is the surrogate signal at $t$ and $R_1$, $R_2$ and $R_3$ are polynomial coefficients. 
...
NiftyRegResp was used to estimate the motion models, using the sum of squared differences as objective function. The surrogate signal passed to NiftyRegResp consisted of both the displacement of the anterior-posterior and the  diaphragm movement from XCAT. Fig.~\ref{fig:mcr} illustrates the NAC motion-compensated reconstructions (MCR) obtained by NiftyRegResp after estimation of the motion models.

\section{Results}
In order to evaluate the motion models, the estimated deformation fields were used to warp the ground truth PET volume. The warped images were then compared to the original volumes $\bold{f}_t$ from XCAT using mean squared errors (MSE) as $\mathrm{mean}((\bold{f}_t-W(\alpha_t) \bold{f}_\mathrm{ref})^2)$. We compared $3$ motion models, calculated from the PET XCAT images (considered to be the ideal case), from nonTOF NAC reconstructions and the TOF NAC reconstruction respectively.  
...
%Figure~\ref{fig:non_tof} shows the difference between the ground truth and the nonTOF derived motion model at end-inhalation, as can be seen there is a great deal of variation between the two images when it comes to the motion of the lesion and liver. 
...
\begin{table}[H]
    \centering
        \caption{Comparison of the MSE for the nonTOF data and the TOF data for all gates plus the mean MSE over all gates.}
    \begin{tabular}{|l|ll|}
    \hline
...
\section{Discussion}
NAC TOF motion models were found to be more robust than NAC non-TOF motion models, both visually and comparing MSE. This was most noticable for the lung lesion. This is likely due to the improved image contrast of NAC TOF images. However, a disadvantage of the TOF reconstruction is that it increases the computational time of the reconstruction.

In future, we will investigate the robustness of the motion model estimation to different noise levels and acquisition duration and evaluate the effect of incorporating the estimated deformations into reconstruction (MCIR).

The estimated deformations can be used as initialisation for joint reconstruction of activity image and motion estimation~\cite{Bousse2016a}, potentially combined with motion model estimation.
