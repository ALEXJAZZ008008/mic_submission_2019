...
\begin{abstract}
Respiratory motion reduces image quality in positron emission tomography. Unless gated CT or MR data are available, motion correction relies on image registration of the PET data. To avoid mis-registration due to attenuation mismatches, most existing methods rely on pair-wise registration of non-attenuation-corrected (NAC) PET images. This is a challenging problem due to a low contrast and high noise. This paper investigates the possibility of using motion models for respiratory motion correction in PET, and in particular whether incorporating time-of-flight information increases the accuracy of the motion models derived from the NAC reconstructed images. XCAT simulations are used for one bed position with a field of view of the lungs and diaphragm. A TOF resolution of 375ps was used. NAC images are obtained using OSEM and used as input for motion model estimation. Different motion models are compared using the XCAT ground truth images. The results indicate that TOF improves the accuracy of the motion model considerably.
...
Surrogate driven motion models attempts to overcome these deficiencies by relating the motion in the data to a surrogate signal~\cite{McClelland2013}.%, for instance, the displacement of the anterior surface and/or the position of the diaphragm.
...
\section{Discussion}
NAC TOF motion models were found to be more robust than NAC non-TOF motion models, both visually and comparing MSE. This was most noticable for the lung lesion in the thoracic cavity. This is likely due to the improved image contrast of NAC TOF images.
